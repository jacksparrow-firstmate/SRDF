...
def parse_gt_txt(filename):
    ...
    with open(filename,'r')as F:
        for obj in F.readlines():
            object_struct = {}
            label = obj[:-2].split(' ')
            bbx = list(map(float,label[0:8]))
            difficult = 0
            object_struct['name'] = 'ship'
            object_struct['difficult'] = difficult
            object_struct['bbox'] = bbx
            objects.append(object_struct)
    return objects

def parse_gt_text(filename):
    label = []
    f,n = filename.split('_')[0][:-3],filename.split('_')[1]
    # filename = f+'gt_img_'+n
    # filename = f + n
    with open(filename, 'r', encoding='utf-8-sig') as f:
        ...
        for obj in objects:
            if len(obj) != 0:
                ...
                object_struct['name'] = 'ship'
                object_struct['difficult'] = difficult
                object_struct['bbox'] = box
                label.append(object_struct)
    return label
...
def voc_eval(detpath,
    ...
    for i, imagename in enumerate(imagenames):
        recs[imagename] = parse_gt_txt(os.path.join(annopath.format(imagename)))
